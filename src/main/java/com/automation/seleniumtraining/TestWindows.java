...
public class TestWindows {
	...
	public static void main(String[] args) {
		...
		// implicit wait - wait for an element to be visible - Applicable to all
		...
		// Special case
		 ...
		 /* while(driver.getWindowHandles().size()==1){ // check for the number of handles
		...
		for (String s : handles) {
			if (!s.equals(main)) {
				...
				// This will wait for a certain expected conditions to happen
				// within the webdriver wait period
				// webwait.until(ExpectedConditions.titleContains("Facebook"));
				String title = driver.getTitle();
				if (title.contains("Facebook")) {
					driver.findElement(By.linkText("Create a Page")).click();
					//webwait.until(ExpectedConditions
						//	.visibilityOfElementLocated(By.cssSelector("div[class=\"facade_text\"]")));
